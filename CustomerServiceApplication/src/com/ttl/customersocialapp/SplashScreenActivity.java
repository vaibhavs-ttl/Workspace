	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			info = manager.getPackageInfo(
					SplashScreenActivity.this.getPackageName(), 0);
		...
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				String getname = settings.getString(name, "");
				String getpassword = settings.getString(password, "");
				...
				if (pref.getBoolean("activity_executed", true)) {
					...
					chkweather = true;
					UserDetails.setUser_id(getname);
					...
					if (connect) {
						...
						try {
							...
							new AWS_WebServiceCall(SplashScreenActivity.this,
									req, ServiceHandler.POST, Constants.user,
