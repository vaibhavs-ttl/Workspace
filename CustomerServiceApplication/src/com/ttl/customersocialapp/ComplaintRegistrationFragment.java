...
public class ComplaintRegistrationFragment extends Fragment {
	private Spinner  spinner_c_area,spinner_c_subarea,spinner_c_problem,spinner_regNo,spinner_dealer;     
	...
	String DIVISIONNAME , CHASSISNO,contact_id , REGNO;
	...
	View v;
	String rowid="";
	String[] lovs ={"Warranty" ,
			"Dealer Service Related",
			"Delivery",
			"Others - Complaint",
			"Parts Related",
			"Product Related",
...
//				+"</SOAP:Envelope>";
      ...
      //  lovs
        ...
        String req = "<SOAP:Envelope xmlns:SOAP=\"http://schemas.xmlsoap.org/soap/envelope/\">"
        		...
        		+"</SOAP:Body>"
        		+"</SOAP:Envelope>";
        	Log.d("req", req + Config.getSAMLARTIFACT());
      		...
      		spinner_regNo.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
    			...
    			public void onItemSelected(AdapterView<?> arg0, View arg1, int pos, long arg3) {
    				...
    				if (pos != 0) {
    					...
    					{
        					String chassis = new UserDetails().getRegNumberList().get(pos-2).get("chassis_num");
    					...
    					/*+ new UserDetails().getRegNumberList().get(pos-2).get("registration_num") +*/
				...
				//	String PAR_ROW_ID =  lst_c_area.get(position-1).getPAR_ROW_ID();
						...
						for (int i = 0; i < lst_c_area.size(); i++) {
							//Log.i("lst_c_area", lst_c_area.get(i).VAL + " "+ lst_c_area.get(i).ROW_ID);
								...
								if(lst_c_area.get(i).VAL.equals(parent.getSelectedItem().toString()))
								{
					...
					//Log.i("CMPL_AREAROW_ID", lst_c_area.get(position-1).ROW_ID);
					String req2 = "<SOAP:Envelope xmlns:SOAP=\"http://schemas.xmlsoap.org/soap/envelope/\">"
							+"<SOAP:Body>"
							+"<GetCompCitiesByType xmlns=\"src.com.CSB\" preserveSpace=\"no\" qAccess=\"0\" qValues=\"\">"
					      +"<CMPL_AREAROW_ID>"+rowid+"</CMPL_AREAROW_ID>"
					    +"</GetCompCitiesByType>"
					  +"</SOAP:Body>"
					+"</SOAP:Envelope>";
      		...
      		spinner_city.setOnItemClickListener(new OnItemClickListener() {
				...
				//	if(position!=0){
					...
					/*String req = "<SOAP:Envelope xmlns:SOAP=\"http://schemas.xmlsoap.org/soap/envelope/\">"
							...
							+ "<CITY>" + lst_c_area.get(position).ROW_ID+spinner_city.getText().toString() + "</CITY>" + "<BU>TMPC</BU>"
							+ "</GetDlrsByCityAndTypeCSB>" + "</SOAP:Body>" + "</SOAP:Envelope>";*/
					String req = "<SOAP:Envelope xmlns:SOAP=\"http://schemas.xmlsoap.org/soap/envelope/\">"
							+ "<SOAP:Body>"
							+ "<GetDlrsByCityAndTypeCSB xmlns=\"src.com.CSB\" preserveSpace=\"no\" qAccess=\"0\" qValues=\"\">"
							+ "<CITY>"+rowid+spinner_city.getText().toString() + "</CITY>" + "<BU>TMPC</BU>"
							+ "</GetDlrsByCityAndTypeCSB>" + "</SOAP:Body>" + "</SOAP:Envelope>";
      	...
      	/*	spinner_city.setOnItemSelectedListener(new OnItemSelectedListener() {
				...
				public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
					...
					String req = "<SOAP:Envelope xmlns:SOAP=\"http://schemas.xmlsoap.org/soap/envelope/\">"
							+ "<SOAP:Body>"
							+ "<GetDlrsByCityAndTypeCSB xmlns=\"src.com.CSB\" preserveSpace=\"no\" qAccess=\"0\" qValues=\"\">"
							+ "<CITY>" + lst_c_area.get(position-1).ROW_ID+spinner_city.getSelectedItem().toString() + "</CITY>" + "<BU>TMPC</BU>"
							+ "</GetDlrsByCityAndTypeCSB>" + "</SOAP:Body>" + "</SOAP:Envelope>";
					new WebServiceCall(getActivity(), req, Constants.GetDlrsByCityAndTypeCSB, new ResponseCallback() {
      		...
      		spinner_dealer.setOnItemSelectedListener(new OnItemSelectedListener() {
    			...
    			public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
    				...
    				String req1 = "<SOAP:Envelope xmlns:SOAP=\"http://schemas.xmlsoap.org/soap/envelope/\">"
    						...
    						+"</SOAP:Body>"
    						+"</SOAP:Envelope>";
    				new WebServiceCall(getActivity(), req1, Constants.GetParUsrPostnByPostTypeAndDivID_CSB, new ResponseCallback() {
    					
    					@Override
    					public void onResponseReceive(Object object) {
    						
    						//String dealercntctNum = (String) object;
    						dealer = (HashMap<String, String>) object;
    						if(TextUtils.isEmpty(dealer.get("mobile")))
    							edit_dealerNo.setText("Not available");
    						else
    							edit_dealerNo.setText(dealer.get("mobile"));
...
//							               +"</SOAP:Envelope>";
						...
						String reg ;
						 if(spinner_regNo.getSelectedItem().toString().equals("Not Available"))
	    					{
					...
					new WebServiceCall(getActivity(), req,Constants.TMPCComplaintsInsertOrUpdate_Input, new ResponseCallback() {
						...
						public void onResponseReceive(Object object) {
							...
							mTracker.send(new HitBuilders.EventBuilder()
			   	        	...
			   	        	.setLabel("ComplaintRegistration")
				            .build());
						...
						//	db.addComplaint(complreg);
                            ...
                            //Save Complaint registration history on Cloud.
                       String req = Config.awsserverurl+"CustomerApp_Restws/customerapp/vehicleServices/insertComplaintHistory";
                                   List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>(8);
                                   
                                   nameValuePairs.add(new BasicNameValuePair("user_id",UserDetails.getUser_id()));
                                   nameValuePairs.add(new BasicNameValuePair("registration_number",complreg.getComplaint_reg_no()));
                                   nameValuePairs.add(new BasicNameValuePair("complaint_number",complreg.getComplaint_no()));
                                   nameValuePairs.add(new BasicNameValuePair("model",complreg.getModel()));
                                   nameValuePairs.add(new BasicNameValuePair("complaint_date",complreg.getComplaint_date()));
                                   nameValuePairs.add(new BasicNameValuePair("primary_complaint_area",complreg.getPrimary_area()));
                                   nameValuePairs.add(new BasicNameValuePair("sub_area",complreg.getSub_area()));
                                   nameValuePairs.add(new BasicNameValuePair("problem_area",complreg.getProblem_area()));

                                   new AWS_WebServiceCall(getActivity(), req,ServiceHandler.POST ,Constants.insertComplaintHistory,
                                                 nameValuePairs,new ResponseCallback(){

                                                       @Override
                                                       public void onResponseReceive(Object object) {
                                                              // TODO Auto-generated method stub
                                                         //     Toast.makeText(getActivity(), "Complaint Registration Details Saved", Toast.LENGTH_LONG).show();
                                                    	   v.getRootView().setFocusable(true);
                                       			  		v.getRootView().requestFocus();
                                                       }
