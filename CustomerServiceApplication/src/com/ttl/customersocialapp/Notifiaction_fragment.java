...
import com.ttl.customersocialapp.R.color;
import com.ttl.customersocialapp.R.drawable;
...
public class Notifiaction_fragment extends Fragment {
	...
	private int[] newlaunch = { R.drawable.newlaunch, R.drawable.newlaunch1,
			R.drawable.newlaunch2, R.drawable.newlaunch3 };
	private int[] customerapp = { R.drawable.customerapp,
			R.drawable.customerapp1 };
	private int[] megacamp = { R.drawable.megacamp, R.drawable.megacamp1 };
	private int[] serviceproduct = { R.drawable.serviceproduct };
	HorizontalScrollView horizontalScrollView;
	NotificationAdapter notificationadp;
	LinearLayout lineardynamic;
	Button dynamicbtn;
	ArrayList<Button> buttongrp = new ArrayList<Button>();
	Button buttonArray[];
	...
	HashMap<String, String> tabList = new HashMap<String, String>();
	private ArrayList<HashMap<String, String>> tabListmap = new ArrayList<HashMap<String, String>>();

	// private ArrayList<String> tabListObject;
	...
	// DownloadImageTask DownloadImageTask;//= new DownloadImageTask(slidingimage);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if(connect){
		String req = Config.awsserverurl+"CustomerApp_Restws/customerapp/notifications/getNotificationsForApp/"
				+ UserDetails.getUser_id();

		new AWS_WebServiceCall(getActivity(), req, ServiceHandler.GET,
				Constants.getNotificationsForApp, new ResponseCallback() {
					...
					public void onResponseReceive(Object object) {
						...
						/*if (HomeFragment.Notificationchk == true) {
							tabList.put("notification_type",HomeFragment.GetNotificationType);
							for (int j = 0; j < AWS_WebServiceCall.notificationhashmaplist
								...
								if (!HomeFragment.GetNotificationType
									...
									tabList.put("notification_type",AWS_WebServiceCall.notificationhashmaplist
						  ...
						  // tabListObject =tabList;
				           ...
				           //   Log.d("get string home1", tabList.get("notification_type").toString());
				              tabListmap.add(tabList);
				              for (int j = 0; j < AWS_WebServiceCall.notificationhashmaplist
				                  .size(); j++) {
				                if (!HomeFragment.GetNotificationType
				              ...
				              Log.d("get data", tabListmap.toString());
				              buttonArray = new Button[tabListmap.size()];
				              loopVariable = tabListmap.size();
				              tabListObject = tabListmap;
				              // tabListObject =tabList;
						...
						for (int j = 0; j < loopVariable; j++) {
							...
							dynamicbtn.setText(tabListObject.get(j).get(
									"notification_type"));
							/*Log.d("get img url in notification", tabListObject
									.get(j).get("banner_image"));*/
							...
							/*
							 * dynamicbtn.setBackgroundResource(
							 * R.drawable.notificationbutton);
							 * dynamicbtn.setTextColor(
					...
					public void onErrorReceive(String string) {
                        ...
                        Toast.makeText(getActivity(), "Notifications not available.", Toast.LENGTH_LONG).show();
			...
			public boolean onKey(View v, int keyCode, KeyEvent event) {
				if (event.getAction() == KeyEvent.ACTION_DOWN) {
					if (keyCode == KeyEvent.KEYCODE_BACK) {
						...
						/*} else {
							Toast.makeText(getActivity(), "not cancled", Toast.LENGTH_SHORT).show();
						}	*/       
						return true;
					}
				}
