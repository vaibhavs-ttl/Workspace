...
import com.ttl.communication.soapConnection;
...
public class WebServiceCall extends AsyncTask<Void, Void, Boolean> {
	...
	String dial_message;
	String webservice;
	ServiceBookingUser user;// = new ServiceBookingUser();
	ArrayAdapter<String> adapter;
	ArrayList<String> list;// = new ArrayList<>();
	ResponseCallback rc;
	ArrayList<Dealer> dealers;// = new ArrayList<Dealer>();
	String dealercntctNum = null;
	String PRI_EMP_ID = null;
	ArrayList<ServiceComplaintDesc> lst_c_desc;// = new
												// ArrayList<ServiceComplaintDesc>();
	ArrayList<ComplaintArea> lst_c_area;
	ArrayList<ServiceHistory> lst_history;
	String err_message = "Data not available.";
	String DelearLocatorActivtiy;
	DelearLocatorData delear = null;
	...
	String fromActivity;
	ArrayList<DelearLocatorData> delearList;
	Dialog dialog;
	ArrayList<ComplaintAndJCDescripti> lst_complaintandJC;
	VehcontactScheduler vehdetails = null;
	ArrayList<VehicleAMC_ParentRow> lst_amcParent;// = new
													// VehicleAMC_ParentRow()
	ArrayList<VehicleAgreement_ParentRow> lst_agreeParent;
	HashMap<String, String> dealerdetails = new HashMap<String, String>();
	...
	protected Boolean doInBackground(Void... params) {
		...
		Log.i("doInBackground", requestString);
		boolean dataReceived = false;
		soapConnection sc = new soapConnection(url, requestString.getBytes(),
				context);
		String xml = sc.getResponce();
		Log.e("Resp: ", xml);
		try {
			...
			if (webservice.equals(Constants.GetComplaintDesc)) {
				...
				while (eventType != XmlPullParser.END_DOCUMENT) {
					...
					case XmlPullParser.START_TAG:
						 ...
						 */else if (tagName.equals("ID")) {
							...
							//Log.d("DIV_ADDRESS_1", delear.delear_DIV_ADDRESS_1);
						} else if (tagName.equalsIgnoreCase("DIV_ADDRESS_2")) {
							delear.delear_DIV_ADDRESS_2 = xmlParser.nextText();
							//Log.d("DIV_ADDRESS_2", delear.delear_DIV_ADDRESS_2);
							...
							//Log.d("DIV_ZIP_CODE", delear.delear_DIV_ZIP_CODE);
						} else if (tagName
						...
						//	Log.d("WEEKLY_OFF", delear.delear_WEEKLY_OFF);
							...
							System.out.println(delear.delear_LONGITUDE.length()
									+ "length");*/
						} else if (tagName.equalsIgnoreCase("err")
								|| tagName.equalsIgnoreCase("faultstring")) {
							dataReceived = false;
				...
				while (eventType != XmlPullParser.END_DOCUMENT) {
					...
					case XmlPullParser.START_TAG:
						...
						if (tagName.equalsIgnoreCase("PRI_EMP_ID")) {
							
							dealerdetails.put("PRI_EMP_ID", xmlParser.nextText());
	...
	//	Log.i("onPostExecute", result + "");
		...
		if (result) {
			if (webservice.equals(Constants.GetCustomerVehicleDetailsCSB)) {
				...
				// rc.onResponseReceive("");
			} else if (webservice.equals(Constants.GetParUsrPostnByPostTypeAndDivID_CSB)) {
				 rc.onResponseReceive(dealerdetails);
			}
		} else {
