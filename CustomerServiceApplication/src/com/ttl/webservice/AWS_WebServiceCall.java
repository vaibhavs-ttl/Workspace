...
public class AWS_WebServiceCall extends AsyncTask<Void, Void, Void> {
	...
	String user_id, password, photoUrl;
	String req, error , response;
	...
	ArrayList<HashMap<String, ArrayList<VehicleAMC_ParentRow>>> amclist = new ArrayList<HashMap<String, ArrayList<VehicleAMC_ParentRow>>>();
	ArrayList<HashMap<String, ArrayList<VehicleAgreement_ParentRow>>> agreelist = new ArrayList<HashMap<String, ArrayList<VehicleAgreement_ParentRow>>>();
	public static ArrayList<String> typelist = new ArrayList<>();
	Bitmap bitmap = null;
	ArrayList<ServiceHistory> lst_history;
	...
	//	System.out.println("Namevaluepair"+this.nameValuePairs.get(0));
	...
	protected void onPreExecute() {
		...
		if (webservice.equalsIgnoreCase(Constants.insertServiceBookingHistory) || webservice.equalsIgnoreCase(Constants.setReadFlag) ) {
			...
			try {
				...
				if (webservice.equals(Constants.user)) {
					if (jsonStr.contains("Forbidden Content")) {
						...
						/*String gcmdetails = c.getString("gcm_details");
						JSONArray gcm = new JSONArray(gcmdetails);
						ArrayList<HashMap<String , String>> objlist = new ArrayList<HashMap<String , String>>();
						for(int i=0; i<gcm.length(); i++)
						{
							JSONObject jgcm = gcm.getJSONObject(i);
							HashMap<String , String> obj = new HashMap<String , String>();
							obj.put("imei_number", jgcm.getString("imei_number"));
							obj.put("gcm_reg_id", jgcm.getString("regid"));
							objlist.add(obj);
							
						}
						
						UserDetails.setGcmlist(objlist);*/
						dataReceived = true;
					}
					...
					if (!(vehicle.equals(""))) {
						for (int i = 0; i < vehicle.length(); i++) {
							...
							String engine_num = type.getString("engine_num");
							...
							String dealer_city = type.getString("dealer_city");
							String insurance_company_name = type
									.getString("insurance_company_name");
							String policy_num = type.getString("policy_num");
							...
							String ew_end_date = type.getString("ew_end_date");
							String ew_start_km = type.getString("ew_start_km");
							...
							details.put("warranty_end_date", warranty_end_date);
							details.put("last_service_date", last_service_date);
							details.put("last_service_kms", last_service_kms);
							...
							details.put("policy_start_date", policy_start_date);
							...
							details.put("vehicle_image_url", vehicle_image_url);
							details.put("contact_last_name", contact_last_name);
							...
							for (int j = 0; j < aggrement.length(); j++) {
								JSONObject agtype = aggrement.getJSONObject(j)
										.getJSONObject("aggObj");
								...
								agreeParent.setMech_reamaing(agtype
										.getString("mech_service_remaining"));
								agreeParent.setTowing_avail(agtype
										.getString("towing_service_availed"));
								agreeParent.setTowing_reamaing(agtype
							...
							for (int j = 0; j < amc.length(); j++) {
								...
								amcParent.setAmc_no(atype.getString("amc_num"));
							...
							/*
							 * Log.d("agreement size", lst_amcParent.size()+"");
							 * 
				...
				//	Log.d("notificaion length", notification.length() + "");
					if (notification.length() != 0) {
						for (int i = 0; i < notification.length(); i++) {
							...
							if (ntf.length() != 0) {
								...
								/*Log.d("latestnotificaion length", ntf.length()
										+ "");*/
								...
								banner_image = ntf.getString("banner_image");
								...
								notificationlist.put("banner_image",
										banner_image);
								notificationhashmaplist.add(notificationlist);
								/*for (int j = 0; j < notificationhashmaplist
										.size(); j++) {
									Log.d("notification string in call",
											notificationhashmaplist.get(j).get(
													"notification_type"));
								}
*/
					...
					JSONArray notification = new JSONArray(jsonStr);
					Log.d(notification.length() + "",
							"latestnotificaion length");
					if (notification.length() != 0) {
						for (int i = 0; i < notification.length(); i++) {
							JSONObject ntf = notification.getJSONObject(i);
							if (ntf.length() != 0) {
					...
					if (notification.length() != 0) {
						for (int i = 0; i < notification.length(); i++) {
							...
							if (ntf.length() != 0) {
								notification_type = ntf
										.getString("notification_type");
								notification_title = ntf.getString("notification_title");
								notification_desc = ntf.getString("notification_desc");
								notification_image = ntf.getString("banner_image");
								kms_run = ntf
										.getString("kms_run");
								read_flag = ntf
										.getString("read_flag");
								...
								getnotification.put("notification_type", notification_type);
								getnotification.put("notification_title", notification_title);
								getnotification.put("notification_desc", notification_desc);
								getnotification.put("banner_image", notification_image);
								getnotification.put("kms_run", kms_run);
								getnotification.put("read_flag", read_flag);
								getnotification.put("ready_to_publish", ready_to_publish);
								getnotification.put("all_cities_flag", all_cities_flag);
					...
					for (int i = 0; i < resultArray.length(); i++) {
						...
						HashMap<String, String> details = new HashMap<String, String>();
						details.put("registration_number", registration_number);
					...
					for (int i = 0; i < resultArray.length(); i++) {
						...
						String model = resultObj.getString("model");
						String booked_date = resultObj.getString("booked_date");
						String booked_time = resultObj.getString("booked_time");
					...
					String email_id = userDetailsObj.getString("email_id");
					...
					for (int i = 0; i < types.length(); i++) {
						...
						history.setODOMTR_RDNG(type.getString("ODOMTR_RDNG"));
					...
					for (int i = 0; i < types.length(); i++) {
						JSONObject type = types.getJSONObject(i);
						String reminderType = type.getString("reminderType");
					...
					for (int i = 0; i < resultArray.length(); i++) {
						...
						getdetails.put("registration_num", registration_num);
	...
	protected void onPostExecute(Void result) {
		...
		if (webservice
						.equalsIgnoreCase(Constants.insertServiceBookingHistory) || webservice
						.equalsIgnoreCase(Constants.setReadFlag)) {
